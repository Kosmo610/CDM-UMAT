...
| 판정 | 건수 | 뜻 |
|---|---|---|
| **OK** | **52** | 문서에 적은 DOI·권·쪽이 전부 일치 |
| **FILLED** | **46** | 비워 두었던 칸이 채워짐 |
| **MISMATCH** | **3** | 문서/CSV의 값이 틀림 |
| **NOTFOUND** | **12** | 이 질의로는 못 찾음 — **아래 (3) 참조. "논문이 없다"는 뜻이 아니다** |
| 계 | 113 | |

#### (1) MISMATCH — 값이 틀렸다 **3건**

| key | 문헌 | Crossref 확정값 | 조치 |
|---|---|---|---|
| **S21** | Jirásek & Bauer, *Comput. Struct.* | `10.1016/j.compstruc.2012.06.006`, **110–111**, 60–78, 2012 | **문서가 옳았다** — 110/111 **합본호**다. `refs_candidates.csv` 쪽을 수정 |
| **A15** | Olaya Gómez & Garnier, *Int. J. Therm. Sci.* | `10.1016/j.ijthermalsci.2020.106740`, **161**, **106740**, **2021** | ★ 권 159 → **161**, 연도 2021, DOI 확정 |
| **B35** | Li et al., CJA **Part II** | `10.1016/j.cja.2014.10.027`, **28**, **314–326**, **2015** | ★ 권 27 → **28**, **2015년**, 쪽수 확정 |
...
NOTFOUND 12건 중 **10건은 문서에 이미 DOI가 있으므로** 재실행 시 1단계에서 확인된다:
`S1`, `A2`, `A3`, `A6`, `A17`, `A30`, `B4`, `B18`, `B29`, `C11`.
DOI를 모르는 **`B16`(Morscher & Cawley, JECS 2002)** 과 **`C23`(레이저 가열 SiC/SiC, *Compos. Struct.*)**
두 건만 3단계 재검색 대상이다.
...
| # | 변경 대상 | 내용 | 근거 |
|---|---|---|---|
| 1 | **제2장 §2.4** | `R''''`를 Hasselman **1963**으로 재귀속. Kingery 1955 · Hasselman 1963 · 1969 세 편 병기 | S11–S13 |
| 2 | **제2장 §2.6.1** | "3D Hashin 확장" 서술 삭제 — Hashin 1980이 이미 3D. 대신 Matzenmiller 1995를 판정식→손상 매핑으로 추가 | S2, S5 |
| 3 | **제2장 §2.6.1** | 강도비 R 정규화에 Liu & Tsai 1998 인용 | S4 |
| 4 | **제3·4장** | Chamis 인용을 NASA TM-83320 주 + SAMPE 병기로 | S6 |
| 5 | **`docs/NOVELTY.md` §A.1** | `k` 부호 프레임을 **"2D vs 3D"에서 "밀봉(>1000 °C) vs 비밀봉(600–1000 °C)"으로 교체.** `k(T_max)` 일반화 검토 | §4.2 |
| 6 | **제4장 §4.9-8** | `σ_y0`·`H_iso`를 GUESS 14개에서 분리 — **"수치 정규화 파라미터"** 별도 분류 | §3.7 |
| 7 | **제4장 §4.9-8** | Y_c·X_c에 **"tow-level 측정법 부재"** 명시 | §3.5 |
| 8 | **제4장 §4.9-6a** | `G_tc` 선택지 (b)를 **"직조 복합재 데이터 근거"**로 승격 (출처 확인 조건부) | §3.6 |
| 9 | **제4장 §4.9-9/11** | **2.34배의 주 원인을 CTE에서 무응력 온도로 재배치.** CONFIG_P의 물리적 정당성 강화 | §4.5 |
| 10 | **제4장 §4.9-3** | 공극률 보정에 Maxwell–Eucken/Landauer 경로 추가 | A17 |
| 11 | **제5장 §5.9.2** | **"PIP C/SiC의 사이클별 잔여물성 데이터 부재"**를 명시적 한계로 | §4.1 |
