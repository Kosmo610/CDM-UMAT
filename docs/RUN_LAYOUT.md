...
순간에 한 번 걸어잠근다. 0 이면 σ11 이 혼자 죽인 것, 1 이면 전단이
혼자 죽인 것.

**SDV17=0 은 두 가지 뜻이라 반드시 `SDV5(=R1T) ≥ 1` 로 걸러야 한다** —
"σ11 단독"과 "1T 모드 미개시"가 둘 다 0 이다. 추출 스크립트에 이
필터를 넣을 것.

차분시험 결과 (`bash verification/verify_diagnostic_inert.sh`):

```
1) 무해성   STRESS/CTAN/SV(1..16)/PNEWDT 18880 값이 V2_7P 와
            유효숫자 17 자리까지 동일             PASS
2) 비공허성 SDV17 이 V2_7P 는 0 건, V2_7D 는 142 건 채워짐  PASS
3) 같은 변형경로에서 XT 만 바꾸면 전단분율이
            0.1096 (XT=421)  →  0.8481 (XT=2835)
```

3번은 **단일적분점 규정변형 경로**라 절대값은 내가 준 축:전단 비율에
