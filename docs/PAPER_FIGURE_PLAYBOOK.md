...
## 재고 현황 (2026-08-12 — **논문 전 그림 확보 완료**)

| 논문 그림 | 상태 | 어디에 |
|---|---|---|
| Fig.3 냉각 잔류응력 | ✅ | `Try_P2\img_S11_*_T*.png` (10) |
| Fig.4 냉각 손상률 | ✅ | `fig4_cooling_damage.png` |
| Fig.5 냉각 손상 컨투어 | ✅ | `Try_P2\img_*_T*.png` (15) |
| Fig.6 승온 불변 | ✅ | `fig6_heating_damage.png` |
| Fig.7/8 승온 23→500 | ✅ | `Try_P2T500\img_*_T*.png` (10+15) |
| Fig.9/10 승온 500→1000 | ✅ | `Try_P2T1000\img_*_T*.png` (10+15) |
| Fig.11/13/15 응력-변형률 | ✅ | `fig11_style__P0/_P1/_P2/_P2T500/_P2T1000.png` |
| Fig.12/14/16 인장 단계 컨투어 | ✅ | `Try_P0/P1/P2/P2T500/P2T1000\img_*_f*.png` (15씩) |
| Fig.A1/A2/A3 히스토그램 | ✅ | `figA1_hist__P0/_P1/_P2/_P2T500/_P2T1000.png` |
| Table 3 세 온도 | ✅ | `summary_vs_paper.csv` |

**PNG 150 장.** 행 수가 그림마다 다르다: `--fig stress` 는 2 행
(기지·얀) → 10 장, `--fig damage` 는 3 행 → 15 장.

**Table 3 / A2 / A3 는 V2_7P 배치라 범례가 논문과 같은
SDV1/SDV2/SDV14 로 찍힌다.** 23 °C 계열(P0/P1/P2)은 V2_7 이라
`SDV_DMT` 식 — 발표에서 섞어 쓸 때 캡션에 밝힐 것.
...
- **콘솔이 비는 것은 정상이다.** 이 환경의 `abaqus cae noGUI=` 는
  ...
  **판정은 항상 로그로 한다:**
  ...
  `exit : 0` 이면 성공. 전체를 보려면 `type make_odb_images_log.txt`.
  방금 돌린 실행의 `exit` 줄이 아직 없으면 렌더링 중이므로 잠시 뒤
  다시 본다.
