...
게이트 전량을 표 3.1로 모은다. 행별 항목 수는 손으로 적은 숫자가 아니라
`verification/check_ch3_numbers.py`가 **매 게이트를 실제로 실행해 센 값**과
행마다 대조하는 값이며, 합계가 어긋나면 커밋이 막힌다.

**표 3.1** 자동 검증 항목 총량 — 게이트 스크립트 × 검증 대상 × 항목 수 (행별 실측 대조)
...
| `yarn_fracture_energy.py --check` | 얀 횡방향 $G_{tt}$·$G_{tc}$ 출처·균열대 적합성 + **실행 덱 이력**(M6 채택 → M7 $G_{tc}$ 철회)과 **두 판정** — $G_{tt}$=0.107은 DEV로 유지(복합재 층간값의 유사 전이, IN 승격 불가), $G_{tc}$=0 확정(메시 부적법 + census 모드 24 ≤0.6 %) | 48 |
...
| `make_property_workbook.py --check` | 물성 현황표가 덱·감사와 어긋나지 않는지 + **표 4.1 렌더링 일치**(본문 표 = 덱 카드 + 출처 감사의 생성 결과, 솔버 슬롯 배제 확인) | 27 |
| `msg_residual_census.py --check` | `.msg` 잔차의 상(phase) 분류·드라이버 구분 + **판정 CSV**(값·근거·판정 동행) + **허용치 대조**(잔차가 허용치의 몇 배인가 → TOLERANCE / EQUILIBRIUM) | 29 |
| `reheat_saturation.py --check` | 하중 없는 재가열 스텝이 손상 상한에 닿는 원인 — 두 가설 기각, 점성 지연 16 %·재분배 84 %, **ETA 감도(0→36 %)** | 38 |
| `reheat_frames.py --selftest` | 재가열 스텝 프레임 101개에서 지연/재분배 분리 — $r_t$ 최고수위가 오른 요소만 재분배(상한), 시간 기준이 아니라 상태변수 기준 | 18 |
| `damage_ceiling.py --selftest` | 손상 상한 0.9 판정 — 공간 분포(밴드 vs 산포)·하중 이전 포화·Voigt 부풀림 상한, 그리고 **순위표 절단과 결합 손상값에 대한 판정 불가 선언** + **인쇄하는 명령이 실제로 실행 가능한가**(damage_map 파서와 대조) | 26 |
| `m6_report.py --selftest` | M6 결과 판독기 — 피크 도달 여부, **접선 정의의 정본**(절대 창·원점 통과·$R^2$ 하한)과 기각된 상대 창의 절단 표류 재실측·**곡선 열을 이름으로 해석**(끼워 넣은 열 흡수) | 26 |
| `m6_verdict.py --selftest` | M6 판정기 — 접선 함수를 **import 로만** 쓰는지 원본 대조, 세 곡선 재적합, 값·근거·판정 CSV + **연화 관문 4b절**(곡선이 $G_f$ 를 공급할 자격이 있나, 임계값은 `homogenize.py` 원본 대조) | 28 |
| `compare_tangent.py --selftest` | ITAN 0/1 수렴 비용 대조 — 답이 같은지를 먼저 판정하고, 다르면 비용행을 읽지 못하게 막는다 | 21 |
| `matrix_audit.py --check` | 9케이스 매트릭스가 통제된 실험인지 — TRS 축은 냉각/동결에서만, 심각도 축은 열 잡 참조에서만 다름을 diff 휘슬리스트로 강제, 파손 주입 시험 + 실행 계획서 생성 + **매니페스트에 빈칸 `<...>`이 없는가**(재시작 45줄이 부모 잡 실명을 덱에서 읽어 채웠는가·체크포인트가 사이클 순서인가·후처리 인자 개수)·**규모/비용은 세어서 적고 벽시계는 지어내지 않는가·메모리 합계 225 GB 초과를 드러내는가** | 56 |
...
| `refs_audit.py --check` | refs/ 전수 — 폐번·고분자기지·인용↔목록·기법 원전·Chamis 식 검증 + 미인용 항목 분류 + [S12]·[S13]·C5–C7 미보유 검증 | 134 |
| `refs_74_75.py --check` | **OA 2편 입고 심사** — `[74]` Li 2019(PLS 를 TRS 지표로 쓰는 논리의 방법론 원전, 기구까지 원문 명시)·`[75]` Xu 2023(사이클 대 잔여강도 선형 열화식, 경향 대조 전용) + **PLS(T) 부호 함정** — C/SiC 는 +71 %, SiC/SiC 는 −31 % 로 재료계마다 방향이 반대이므로 `[75]` 의 PLS 방향이 검증 표적으로 새어 들어가지 못하게 막는다 | 33 |
| `gf_temperature.py --check` | $G_f(T)$ 방향(Snead Fig.14)과 $A$ 표류 한계 + **재소싱된 M6 카드는 g0가 39.4 % 상승**(옛 카드는 5.9 % 하락 — 부호가 반대)·**Gf 거부 시 오차 방향이 비보수적으로 뒤집힘**·**39 %의 단일 원인이 X(T)임을 인자별 교체로 확정**·**두 앵커의 2.9배 불일치를 원문(진공 대 대기·산화 진술)으로 중재하고 비보수 판정을 철회** | 60 |
| `pls_validation.py --check` | 비례한도의 TRS 민감도·정의 취약성·선형구간 비 + **전제의 1차 원전 승격**(`[74]` 보유 fulltext — 기구 문장 축자 재독, Liu 확보 과제는 3D SiC/SiC 한정으로 강등) + **적용 경계**(PLS(T) 부호는 재료계 고유 — `[75]` SiC/SiC 는 −31 %) | 53 |
...
| `thermal_cycling_dataset.py --check` | 반복 열충격 전 데이터·심각도 역설·임계온도 공백 + [68] 전문 정정·논문 수 + **모델의 두 절반이 서로 다른 분위기에 묶여 있음**(단조=진공 [5], 사이클=공기)·[43]이 한 재료 네 분위기의 민감도 출처임 + **무포화 대조**(`[75]` 선형 열화식 재유도 — 감쇠 없는 일정 감소분, DATA 밖 격리, §C 포화 주장 비오염) + **표 2.1 렌더링 일치**(본문 표 = DATA 생성 결과, 한 글자 단위) | 73 |
| `atmosphere_census.py --check` | **하중지지 출처 전수의 시험 분위기** — 인용문이 PDF에 실재하는가·단조=진공/사이클=산화 사슬 일관성·자동 훑기의 거짓 양성 3건(Argonne≠argon 등)을 교훈으로 고정·미기재는 미기재로·**refs 색인의 분위기 열이 census와 어긋나지 않는가** | 30 |
| `atmosphere_verdicts.py --check` | **a1의 분류 판정** — a2 census 위에 얹는 귀속 심사. a2 인용 3건의 PDF 재독·[5] 대 [10] 36.2 %p 간극의 **상온 대조(1.76배)와 [10] 자신의 무산화 예측(21.8 %)** 재유도·「대기 대가 2/3」 귀속 철회가 본문에 반영됐는가·[12] 등급 불변과 **온도 미기재** 가정의 명문화·[67] 3D 편조 → 표적 아닌 민감도 + **B-4 답변**(얀 $X(T)$ 출처 = Sauder [8] $\sigma_0$, 그리고 옳은 앵커 대비 12.6 %p **미달**) | 52 |
...
| `retune_deck.py --check` | 덱 재튜닝(카드 슬롯·스텝·M6 카드·균열대 허용성·보정가이드 전사 대조·κ 항등식과 거부·ITAN 블록 슬롯과 미지정 시 바이트 동일성·**M7 프리셋(슬롯 35 = 0 + stabilize 한 단계)과 ftol 소진 확인**·**M8 `--eps`/`--maxinc`(구간 ×3, 증분당 변형률 보존)**) | 187 |
...
| `check_ch6_numbers.py` | 제6장 검증표적 vs 사이클 데이터셋 재유도 (T5 정정 + §6.2.3 확정표 + **§6.6.2 부등호 재유도** — 거시 $\ell_e$·피크 이후 소산에서 $A<2$를 세우고 RT23은 판정 불가로 고정, **§6.6.2-a 소절까지 부호 금지 확대** — 소절이 부모 절에 상한을 귀속하지 못하게, 다만 과거 진술의 기록은 남기게) | 72 |
...
| `check_ch7_numbers.py` | 제7장 결론 경계 — 결과 없는 결론 6개의 근거·결과 의존 구역의 완료어 금지 + 한계 14개(횡방향 압축 「미공개」 판정의 두 다리, **잔여강도 하한** — 제6장과 같은 부등호를 인용하는지 교차 확인 포함)와 소성 몫 재유도 | 54 |
| `check_chapter_flow.py` | 제1~5장 유기적 연결성 (본문 `§` 상호참조 전수 해석 포함 — 그림 자리 상자의 참조도 검사 대상) | 221 |
| `review_inbox.py --check` | 리뷰 브랜치(a3) 수신 — 원문 PDF 보유 확인·저자 명단·Zhang의 내부 참고문헌 30·32·33번·Ge Table 3 파괴에너지·두 초안 계보 대조 | 29 |
| `check_manuscript_citations.py` | **제출본 범위** 인용 감사 — 표 등재분의 본문 인용·본문 마커의 표 등재·무인용 동향 주장·[S*] 보유 진술 + **일곱 장에 병합 충돌표시가 없는가(문서 온전성)** + **표 번호 체계**(R24 — 캡션이 표 위에 있는가·장 번호·등장 순서 오름차·본문 호출·죽은 참조, 「목표 0.4349」류 한글 오탐 배제) | 56 |
| `prerun_gate.py --check` | 최소 해석 계획 — §5.7 매트릭스 파싱·단계별 관문/소유자/낭비조건·보정 의존성(자리표 대 미보정 구분)·산출물 부재와 잡 차단의 분리·$G_{tc}$ 메시 적법성 | 44 |
| `pending_slots.py --check` | `[결과 대기]` 슬롯 8개 — 단계·소유자·차단/부분 분류, 부분 채움에 「1차 결과」 표기 강제 + **자리표 12개의 방향** — 값이 확정되면 무엇이 어느 방향으로 움직이는가를 실측 자코비안에서 재유도하고, 말할 수 없는 것은 이유와 함께 「미정」으로 고정, **방향마다 무엇에 견준 것인지(기준선) 동반 강제**, $G_{tt}$·$G_{tc}$ 미정 사유가 실행 덱(M8) 실제 슬롯과 맞는지 대조 | 48 |
| `check_gf_scale_transfer.py` | $\bar G_f$의 소산분 분해·두 규약의 일치·덱 생성기 관문·**면적 함수가 NumPy 개명에 견디는가** | 82 |
| `card_pipeline_rehearsal.py --check` | **이음새 전구간 리허설** — RVE 실측곡선 → 거시카드 → 9덱 → 축 감사. **연화하지 않은 곡선의 $G_f$ 거부**(연화량·피크이전 몫 두 조건, 서로 독립)·손상 상한 두 스케일 일치·카드 검증기 수용 | 52 |
...
| `check_printed_commands.py` | 스크립트가 **인쇄하는 명령**의 실행 가능성 — 파일 존재·`abaqus python`/일반 `python` 적합성·상대 파서 대비 인자 개수 (자기 예외는 1개로 고정) — 매니페스트 생성기의 새 명령까지 자동 포함 | 26 |
| **합계** | | **3774** |
...
8. **★ 손상 상한 $d_{max}=0.9$가 하중 이전에 이미 걸린다 — C1 비교의 전제를
   건드린다.** 이것은 코드의 결함이 아니라 **카드 선택의 결과**이지만, 제2장
   §2.8.3이 세운 주 주장의 측정 가능성에 직접 닿으므로 여기 적는다.

   8.1 사실. M6 T500의 상별 최악부 손상값 42개가 상한에 앉아 있고, **그중 12개는
   `Heating_to_500C` 스텝 끝에 이미 그렇다.** 그 스텝은 역학 하중을 전혀 걸지
   않는다 — 재가열만 한다. 냉각 스텝 끝의 기지 최대 손상은 0.344였으므로,
   **재가열 구간에서 0.344 → 0.9(상한)로 올라간 것**이다(제4장 §4.9-0d).

   8.2 왜 C1에 닿는가. §2.8.3의 **B(1회 냉각 후 동결)와 C(손상에 따라 재분배)의
   차이는 오직 손상 구동 재분배**다. 그런데 **상한에 닿은 요소는 더 이상 강성을
   잃지 못하므로 더 재분배하지도 못한다.** 즉 재가열만으로 상한에 닿는 요소가
   생기면 **B와 C의 차이는 그 요소들에서 구조적으로 0이 된다.**

   8.3 그래서 어느 방향으로 틀리는가. **차이를 과소평가하는 쪽이다.** 상한이
   재분배를 막으므로 C가 B에 가까워진다. §2.8.3은 "차이가 작으면 실무 단순화가
   정당하다"는 결론도 성립한다고 적었는데, **그 결론만은 이 한계를 확인하기 전에
   내릴 수 없다** — 작게 나온 차이가 물리인지 상한인지 구별되지 않기 때문이다.
   차이가 크게 나오는 경우에는 이 한계가 결론을 흔들지 않는다(상한은 차이를
   키우는 방향으로 작동하지 않는다).

   8.3-a **원인 추적 — 재가열이 손상을 「만드는」 것이 아니다** (2026-08-16,
   `verification/reheat_saturation.py`, 솔버 불요). 8.1의 사실만으로는 원인을 알 수
   없으므로 파이썬 미러(`verify_constitutive.matrix_point`)로 되짚었다.

   먼저 **쉬운 설명 둘은 틀렸다.**
   (i) "고온에서 강도가 떨어져서"가 아니다 — 기지 카드는 **상수 25개이고 온도표가
   아예 없다.** $X_t$는 어느 온도에서도 310 MPa다(온도의존은 얀 카드에만 있다).
   (ii) "재가열이 하중을 건다"도 아니다 — 기지 CTE 4.5e-6이 얀 축방향 1.07e-6보다
   크므로 **냉각이 기지를 당기고 재가열은 그 인장을 푼다.** 2상 평행봉으로
   23 °C에서 0.191 %, 500 °C에서 0.102 %로 **단조 감소**한다.

   그런데도 미러에서 손상은 **오른다.** 이유는 구성식 두 줄이다.

   ```
   RT  = max(sv[2], FIT)              하중함수는 최고수위 기록 — 내려가지 않는다
   gam = dtime / (ETA + dtime)        손상은 목표까지 매 증분 일부만 간다
   d   = d + gam * (target(RT) - d)
   ```

   즉 **목표는 냉각이 이미 정해 놓았고, 점성 정규화(ETA = 0.05)가 그 목표에 도달하는
   것을 늦춘다.** 재가열 구간은 변형률이 줄고 $r_t$가 1.2665로 **한 자리도 안 움직이는데도**
   손상이 0.3443 → 0.4348로 오른다. **새 구동력이 아니라 밀린 것이 도착하는 것이다.**

   > **그러나 이것만으로는 부족하며, 그 점이 중요하다.** 점성 지연이 설명하는 몫은
   > 0.0906 — 상한까지 남은 0.5557의 **16 %뿐**이다. 그 뒤 지연은 얼어붙은 $r_t$의
   > 목표 0.4349에서 **멈춘다.** 재료점 하나로는 더 갈 곳이 없다.
   > **나머지 84 %는 요소 사이의 하중 재분배로만 설명된다** — 무른 요소가 이웃에게
   > 하중을 넘기고, 이웃이 자기 문턱을 넘고, 다시 물러지는 되먹임이다.

   > **그래서 8.3의 관문이 약해지지 않는다 — 오히려 강해진다.** 재가열 상승의 대부분이
   > **수치 인공물이 아니라 §2.8.3이 B–C 차이로 정의한 바로 그 재분배**이기 때문이다.
   > 논문이 재려는 기구가 **하중을 걸기도 전에 이미 돌고 있고 이미 상한에 닿아 있다.**
   > (2상 평행봉은 $r_t$ = 0.8853으로 문턱에 못 미치므로, 미러는 측정된 냉각 손상
   > 0.344에 **배율 2.2752로 보정**하여 구동했다. 보정은 크기만 바꾸고 재가열 구간이
   > **단조 감소한다는 형상은 바꾸지 않으며, 논거는 그 형상이다.**)

   8.3-b **점성 지연의 몫은 ETA가 정한다 — knob의 값이 아니라 값어치를 적는다.**
   ETA = 0.05는 `card_gap_triage`가 이미 **KNOB**(솔버 파라미터, 물성 아님)으로
   분류한 값이다. 위 16 %가 바로 그 knob의 값어치다 — 같은 구동에서 ETA만 바꾸면:

   | ETA | 0 | 0.025 | **0.05 (카드)** | 0.1 | 0.2 |
   |---|---|---|---|---|---|
   | 지연 몫 | 0 % | 8.4 % | **16.3 %** | 26.9 % | 36.0 % |

   즉 **재가열 상승 중 「수치적」인 몫은 ETA로 조절되는 양**이고, 재분배 몫은
   그 나머지다. 그래서 재분배 몫의 확정은 ETA에 기대지 않는 측정으로 한다 —
   `postprocess/reheat_frames.py`가 재가열 스텝의 프레임 101개에서 **요소별
   최고수위 기록 $r_t$(SDV3)가 올랐는가**로 가른다. $r_t$가 얼어 있는 요소의
   상승은 순수 지연이고, 거시 변형률이 감소하는 스텝에서 $r_t$가 오른 요소는
   이웃에게서 하중을 받은 것 — **재분배 상한**이다. 시간 기준이 아니라
   상태변수 기준이므로 두 기구가 겹쳐도 갈린다.

   8.4 무엇을 해야 닫히는가. 세 가지 중 하나면 된다.
   (a) `volfrac_at_cap`으로 상한에 앉은 **부피분율**을 재고, 그것이 작으면
   재분배 손실이 무시할 만하다고 정량으로 말한다 — `postprocess/damage_ceiling.py`
   가 그 판정을 하며 **후처리 재실행 한 번**이면 된다(해석 불요).
   (b) $d_{max}$를 올린 대조 잡 하나를 돌려 B–C 차이가 얼마나 커지는지 본다.
   (c) 재가열 포화가 무응력 온도 1050 °C와 구성재 CTE의 조합에서 오므로,
   §4.9-9·§4.9-11의 CTE 감도와 함께 판정한다.
   (d) **가장 싼 것 — 재가열 스텝을 프레임별로 읽는다.** 덱이 스텝당 필드
   프레임 101개를 쓰는데 `damage_map.py`는 `frames[-1]` 하나만 읽는다. 그 중간
   프레임들이 **점성 지연 몫과 재분배 몫을 이미 갈라 놓고 있다** — 지연은 140증분에
   닫히므로 스텝 초반에 끝나고, 그 뒤로도 오르는 것은 전부 재분배다. 해석 불요.

   > **지금 지켜야 할 규율.** C1의 결론을 "차이가 작다 → 단순화가 정당하다"로
   > 쓰려면 **8.4(a)를 먼저 통과해야 한다.** 차이가 크게 나오는 경우에만 이 관문
   > 없이 결론을 쓸 수 있다.
