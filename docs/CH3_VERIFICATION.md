...
| `check_chapter_flow.py` | 제1~5장 유기적 연결성 (본문 `§` 상호참조 전수 해석 포함 — 그림 자리 상자의 참조도 검사 대상) | 210 |
...
> **[그림 3.1 자리] V3_0 한 증분의 상태 갱신 순서도**
> 내용: 변형률·온도 입력 → 유효응력 → 얀 Hashin 4모드 / 기지 판정(§3.2.1–3.2.2)
> → 손상 진전 + 점성 정규화(§3.2.3–3.2.4) → 균열 닫힘(§3.4.2) → 사이클
> 항(§3.4.4) → 응력·야코비안 반환. 각 상자에 대응 SDV 번호를 병기.
> 재료: 수식·코드 구조만으로 작도. / 시점: **지금 제작 가능**
...
> **[그림 3.2 자리] 지수 연화 곡선과 균열대 에너지 보존**
> 내용: (a) 연화 법칙 $d(r)$ 곡선 — $A$ 몇 개 값(클수록 급락); (b) 같은 $G_f$
> 에서 요소 크기 $l_e$가 다르면 $A$가 달라져 응력-변형률 곡선 아래 면적(소산
> 에너지)이 보존됨을 겹쳐 그림.
> 재료: §3.2.3 수식 + 카드 값. / 시점: **지금 제작 가능**
...
> **[그림 3.3 자리] $\mathrm{sign}(I_1)$ 스위치의 tanh 정규화**
> 내용: 활성 가중 $w(I_1)$ 곡선 3개 — $H_{smo}=0$(계단) · 0.02 · 0.1 겹침.
> $I_1=0$ 부근 야코비안 점프가 78.0×에서 2.3×로 줄어드는 것을 주석으로.
> 재료: §3.4.3 수식 + §3.5의 측정 배율. / 시점: **지금 제작 가능**
...
> **[그림 3.4 자리] Fortran↔Python 교차검증 편차 분포**
> 내용: 114개 재료점 상태의 상대편차 히스토그램(로그 가로축, 최대 5.1×10⁻¹⁵)
> — 두 독립 구현의 차이가 기계 정밀도 수준임을 한눈에.
> 재료: `cross_check_fortran.py` 출력. / 시점: **지금 제작 가능**
