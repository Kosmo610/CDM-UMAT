...
| `pending_slots.py --check` | `[결과 대기]` 슬롯 8개 — 단계·소유자·차단/부분 분류, 부분 채움에 「1차 결과」 표기 강제 + **자리표 12개의 방향** — 값이 확정되면 무엇이 어느 방향으로 움직이는가를 실측 자코비안에서 재유도하고, 말할 수 없는 것은 이유와 함께 「미정」으로 고정, **방향마다 무엇에 견준 것인지(기준선) 동반 강제** | 45 |
