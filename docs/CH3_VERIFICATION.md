...
3. **패치 시험(patch test)이 없다.** 균질 변형장이 정확히 재현되는지의 단일요소·
   다요소 시험은 아직 수행하지 않았다. 주기경계조건 구현의 독립 검증으로서
   추가하는 것이 바람직하다.
4. **C3D4 1차 사면체**는 상수 변형률 요소로 연화 문제에 가장 불리하다. 현 단계에서
   교체하지 않으며, 한계로 명시한다.
5. **크랙밴드 정규화의 메시 독립성을 실제로 보이지 않았다.** 이론적으로는 보장되나,
   요소 크기를 바꾼 두 해석의 소산에너지가 같음을 보이는 시험이 필요하다.
