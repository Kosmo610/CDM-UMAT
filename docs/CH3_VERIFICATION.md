...
| `thermal_cycling_dataset.py --check` | 반복 열충격 전 데이터·심각도 역설·임계온도 공백 + [68] 전문 정정·논문 수 + **모델의 두 절반이 서로 다른 분위기에 묶여 있음**(단조=진공 [5], 사이클=공기)·[43]이 한 재료 네 분위기의 민감도 출처임 + **무포화 대조**(`[75]` 선형 열화식 재유도 — 감쇠 없는 일정 감소분, DATA 밖 격리, §C 포화 주장 비오염) | 69 |
...
| `check_chapter_flow.py` | 제1~5장 유기적 연결성 (본문 `§` 상호참조 전수 해석 포함 — 그림 자리 상자의 참조도 검사 대상) | 219 |
| `review_inbox.py --check` | 리뷰 브랜치(a3) 수신 — 원문 PDF 보유 확인·저자 명단·Zhang의 내부 참고문헌 30·32·33번·Ge Table 3 파괴에너지·두 초안 계보 대조 | 29 |
| `check_manuscript_citations.py` | **제출본 범위** 인용 감사 — 표 등재분의 본문 인용·본문 마커의 표 등재·무인용 동향 주장·[S*] 보유 진술 + **일곱 장에 병합 충돌표시가 없는가(문서 온전성)** | 22 |
