...
| `retune_deck.py --check` | 덱 재튜닝(카드 슬롯·스텝 키워드) | 53 |
...
1. **메시 수렴성 검증이 아직 없다.** 현 단계는 탐색용 거친 RVE 메시(26 452 C3D4)를
   ...
   수렴성 검증이 필요하다.
2. **요소 품질.** 현 메시의 4.5 %(1185개)가 Abaqus 요소품질 기준에 미달하며,
   **전부 기지 요소**이다(얀은 0개). 최악 품질 0.00089(권장 > 0.02), 최소 이면각
   0.313°(권장 > 10°)이다. 이는 직물 표면 사이 좁은 틈을 채우는 과정에서 생기는
   것으로, 메시 재생성 시 개선 대상이다.
...
- 총 **347개 항목**이 자동으로 검증되고, 커밋 전 전수 통과가 강제된다.
