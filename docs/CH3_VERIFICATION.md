...
| `matrix_audit.py --check` | 9케이스 매트릭스가 통제된 실험인지 — TRS 축은 냉각/동결에서만, 심각도 축은 열 잡 참조에서만 다름을 diff 휘슬리스트로 강제, 파손 주입 시험 + 실행 계획서 생성 + **매니페스트에 빈칸 `<...>`이 없는가**(재시작 45줄이 부모 잡 실명을 덱에서 읽어 채웠는가·체크포인트가 사이클 순서인가·후처리 인자 개수) | 47 |
