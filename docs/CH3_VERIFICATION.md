...
| `check_chapter_flow.py` | 제1~5장 유기적 연결성 (본문 `§` 상호참조 전수 해석 포함 — 그림 자리 상자의 참조도 검사 대상) | 215 |
...
| `prerun_gate.py --check` | 최소 해석 계획 — §5.7 매트릭스 파싱·단계별 관문/소유자/낭비조건·보정 의존성(자리표 대 미보정 구분) | 30 |
