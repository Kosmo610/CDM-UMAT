...
> **[그림 5.1 자리] 사이클 손상 법칙의 형상**
> 내용: (a) $\Delta d_{cyc}$의 구동항·문턱 구조(문턱 아래 0, 위에서 단조항과
> 곱셈 결합); (b) $C(T_{max})$·$k(T_{max})$ 곡선 — 900–1200 °C 미측정 창을
> 빗금으로 표시(§5.2.2, 심각도 역설과 연동).
> 재료: 카드 값 + `retune_deck.py`. / 시점: **지금 제작 가능**
...
> 심각도 역설 — refs/[03]의 $\Delta T$=600 °C 사이클이 refs/[02]의
...
> **[그림 5.2 자리] 거시 시편 형상과 bias 메시**
> 내용: 문헌 치수의 시편 형상 + 두께 방향 bias 메시 단면 — 급랭 초기 열경계층
> 두께와, 최소 요소가 그 층을 몇 개로 나누는지를 주석으로.
> 재료: `make_macro_thermalshock.py` 생성 수치. / 시점: **지금 제작 가능**
...
> **[그림 5.3 자리] 급랭 경계조건 — $h$ 역산과 Biot 사다리**
> 내용: (a) 역산한 $h$로 계산한 표면·중심 온도 이력(급랭 구간)과 문헌의 냉각
> 시간 점; (b) $Bi$=0.05/1/5 사다리의 두께 방향 온도 프로파일 3장 비교.
> 재료: `quench_calibration.py` + 열 잡 1회분. / 시점: (a) **지금 제작 가능**, (b) 열 잡 후
