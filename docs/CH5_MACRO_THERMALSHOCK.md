...
#### 5.4.2-a 사다리를 실제로 돌린 결과 — 기여 C2의 실측 근거

**아래는 예측이 아니라 측정이다.** 세 심각도의 열전달 잡을 실행하고
`postprocess/extract_thermal_profile.py` 로 판독한 값이며, 정본은
`data/results/macro_heat_ladder.csv` 다. 본문의 숫자는 그 파일에서 읽어
대조된다(`verification/check_ch5_numbers.py`).

| 수준 | $Bi$ | 최대 구배 | 그 시편 낙차 대비 | 균일 온도 가정 |
|---|---|---|---|---|
| **L** | 0.05 | 19.8 K | **7.0 %** | **성립** |
| **M** | 1.00 | 221.2 K | **36.9 %** | 아슬아슬 |
| **H** | 5.00 | 433.3 K | **72.2 %** | **깨진다** |

**이것이 C2가 주장하는 전부이며, 그 이상은 주장하지 않는다.** 균일 온도
가정[17]은 $Bi \approx 0.05$ 에서 사실상 옳고($7.0\,\%$), $Bi = 1$ 에서
흔들리며($36.9\,\%$), $Bi = 5$ 에서 무너진다($72.2\,\%$). 선행연구가 틀렸다는
것이 아니라 — 그들은 $Bi \ll 1$ 에 있었다(§5.4.1) — **어디서부터 틀려지는지에
숫자를 붙였다**는 것이다.

![](figures/fig_5_4_heat_ladder.png)

**그림 5.4** (a) 세 심각도의 측정된 두께방향 구배를 그 시편의 낙차로 나눈 값.
점선은 문헌이 정한 문턱이 아니라 **이 데이터 자신이 「성립」에서 벗어나는 자리**이며,
CSV의 판정 열에서 읽는다. (b) 같은 세 잡의 열경계층 깊이와 그 안에 든 요소 수 —
**구배는 메시가 푼 만큼만 결과다.** 두 패널의 모든 숫자는
`data/results/macro_heat_ladder.csv` 에서 읽으며 그림 파일에 손으로 적힌 값은 없다.

세 잡 모두 열경계층을 요소 5~7개로 분해했고(`space_verdict`), 구배 피크는
첫 프레임보다 훨씬 뒤에서 잡혔다(`time_verdict`). 후자는 우연이 아니다 —
2026-08-11 이전의 첫 증분 규칙 $t_q/50$ 은 refs/[3] 의 피크 시각(0.30 s)보다
길어서, **논문의 헤드라인 수치를 그 사건이 끝난 뒤에 처음 표본추출**하고
있었다(§5.8).

> **심각도 H 는 제2장 §2.5.4 의 심각도 역설과 직접 이어진다.** 그 논의는
> 손상률이 $\Delta T$ 가 아니라 **사이클 최고 온도**를 따른다는 것이었고,
> 여기서 확인되는 것은 그 논의가 성립하는 **구배 조건의 경계**다. $Bi = 5$
> 에서는 시편 내부가 표면과 400 K 이상 다르므로, "이 사이클의 최고 온도"가
> 위치마다 달라진다 — $f_C$ 를 SDV 29 `TWMAX` 로 **요소마다** 평가해야 하는
> 이유가 여기서 정량적으로 드러난다(§5.2.1).
...
전체 해석 매트릭스를 표 5.1로 확정한다. 축과 개수는
`abaqus/matrix_audit.py`가 실행 계획서와 함께 검증한다 — TRS 축은 냉각/동결
카드에서만, 심각도 축은 열 잡 참조에서만 달라야 통제된 실험이다.

**표 5.1** 해석 매트릭스 — 단계(S0–S5) × 잡 종류 × 개수, 3 심각도 × 3 TRS = 9케이스와 열 잡 공유

| 단계 | 잡 종류 | 개수 | 근거 |
|---|---|---|---|
| **S0** | 선행검증 | **3** | §5.8의 셋. 각각 분 단위이며 뒤의 전부를 지킨다 |
| **S1** | RVE (유효물성·보정) | — | 거시 카드가 여기서 나온다. 제4장 |
| **S2** | 열전달 | **3** | 심각도당 1개, 모든 TRS 케이스가 공유 |
| **S3** | 역학 (CONFIG_P) | **9** | 3 심각도 × 3 TRS |
| **S4** | 역학 (CONFIG_V 대조) | **3** | 중간 심각도 × 3 TRS — 순위 불변 확인(§5.6.1) |
| **S5** | 잔여강도 (restart) | 9 × 체크포인트 수 | 부모 역학 잡 이후 |

**단계를 번호로 부르는 이유는 순서가 결론의 유효성을 정하기 때문이다.** 각 단계에는
"이것이 안 되어 있으면 이 잡은 버려진다"는 조건이 하나씩 붙으며, 그 목록은
`verification/prerun_gate.py`가 들고 있다(§5.7.1).

### 5.7.1 어떤 결론이 보정 수렴을 기다려야 하는가

매트릭스를 언제 돌릴 수 있는지는 **결론마다 다르다.** 기여 C1~C4는 세 케이스가
**같은 카드를 공유하는 비교**이므로 카드의 계통오차가 순위에서 상쇄된다 — 보정이
수렴하기 전에도 답이 나온다. 반면 검증 표적 T1~T6은 문헌 수치와 **맞대는** 것이므로
보정 없이는 무의미하다.

| 결론 | 보정 수렴 | 근거 |
|---|---|---|
| C1 TRS 3케이스 차이 | **불필요** | 같은 카드 공유 → 계통오차 상쇄 |
| C2 균일 온도 가정의 한계 | **불필요** | $h$를 $Bi$에서 유도하므로 $\bar k_3$가 바뀌어도 $Bi$는 정확(§5.4.2) |
| C3 반사이클 비대칭의 **부호** | **불필요** | 크기는 필요 |
| C4 파손기준 순위 | **불필요** | 같은 응력장, 하중배수 정규화(§5.2.4) |
| T1–T6 절대 표적 | **필요** | 문헌 수치와 직접 비교 |
| $k$ 부호의 1300 °C 외삽 | **필요** | 보정된 $k$의 외삽이다 |

> **★ 이 논거에는 바닥이 있다.** 상쇄가 성립하려면 카드가 **덜 맞춰진 그 재료**여야
> 하며, **자리표(placeholder)여서는 안 된다.** 자리표면 손상이 아예 발동하지 않아
> 세 케이스가 동일해질 수 있고, 그때 "차이가 작다"는 결과가 아니라 **인공물**이다.
> 자리표는 계통오차가 아니라 다른 재료다. 따라서 S3의 관문은 "보정 수렴"이 아니라
> **"카드가 자리표가 아닐 것"**이다.
