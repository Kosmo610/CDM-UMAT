...
- **제목·저널·권·쪽은 `refs/[05]` 참고문헌부 30번에서 그대로 읽은 것**이다(1차 확인).
  원문 인쇄는 세 번째 저자를 소문자 `G. liu` 로 찍었는데 이는 원문 조판 오류로 보이며,
  정규 표기는 `G. Liu` 다.
- **DOI는 발행처에서 직접 확인하지 못했다.** 이 환경에서 `doi.org`·`api.crossref.org`
  접속이 막혀 있어 **웹 검색 2건이 일치하는 것까지만** 확인했다. 신뢰등급은
  **`search-verified`** 이며, PDF를 받으면 표제면에서 대조해 `fulltext` 로 올린다.
