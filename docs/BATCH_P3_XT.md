...
## 실측 — P 계열 덱은 얀 상수 **31 개**다 (2026-08-12, 준비 중 확인)

`setup_p3.py` 미리보기가 사용자 PC 의 실제 덱을 읽고 찍은 값이다.
문서에 적혀 있던 추정(38 상수, 기지 `*Depvar` 20)은 **틀렸다.**

| 항목 | 문서 추정 | **실측** |
|---|---|---|
| 얀 `*User Material` | 38 | **31** |
| 기지 `*Depvar` | 20 | **14** |
| 얀 `PROPS(32)=GF1T` | 0.03962 | **없음** |

**상수가 31 이면 `IF (NP.GE.32) GF1T=P(32)` 가 거짓이라 GF1T=0,
즉 얀 종방향 크랙밴드가 꺼져 있다.** `A1TEFF = A1T = PROPS(18)`
고정이다. P 계열 전체가 그렇게 돌았다.

### 이것이 P3 에 **유리하다**

크랙밴드가 켜져 있었다면 XT 를 바꿀 때 연화지수까지 같이 움직인다:

```
g0 = XT^2/(2*E1),   A1TEFF = 2*g0*le/(GF1T - g0*le)
XT=421  -> g0=0.348, le=0.057 -> A1TEFF = 2.0
XT=2835 -> g0=15.76             -> GF1T < g0*le -> A1TEFF = 50 (클램프)
```

**연화지수가 2.0 → 50 으로 같이 튄다. 변수가 둘이 되어 배치가
무효가 된다.** 크랙밴드가 꺼져 있으므로 P3 는 파손 기준 하나만
바뀐다 — 이 배치에서 가능한 가장 깨끗한 단일 변수다.

대신 메쉬 의존성은 그대로 남는다. 그것은 논문에 없는 개선이라
사용자 본인 연구로 미룬 항목이다 (CLAUDE.md §8).
