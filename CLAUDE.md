...
### 7-1) 전문용어는 **`원문(쉬운 뜻)`** 으로 적는다 (사용자 지정, 2026-08-21)

사용자는 복합재 전공자가 아니다. **모든 채팅과 모든 `.md`에 예외 없이 적용한다.**

```
✅  off-axis(섬유 방향에서 비스듬히 기울여 자른) 시편을 당기면 ~
✅  CVI(Chemical Vapour Infiltration, 기체를 스며들게 해 굳히는 제조법)
✅  할선(원점에서 그은 기울기) 기울기로 재면 ~
❌  off-axis 시편을 당기면 ~
❌  비스듬히 자른 시편(off-axis)을 당기면 ~   ← 순서가 반대다
```

지켜야 할 것:

- **순서는 항상 `원문(쉬운 뜻)`.** 원문이 앞이다. 사용자가 논문에서 그 단어를 다시
  만났을 때 알아보는 것이 목적이므로, 원문을 괄호에 넣으면 목적을 잃는다.
- **전문용어에만 붙인다.** 일반 단어(`시편`, `온도`, `실험`)에는 붙이지 않는다.
  붙일지 말지 헷갈리면 기준은 하나다 — **학부 기계공학 2학년이 아는 말인가.**
- **한 답변 · 한 문서 안에서 첫 등장 때 한 번만.** 두 번째부터는 원문만 쓴다.
  긴 문서는 장(챕터)이 바뀌면 다시 한 번 붙여도 된다.
- **괄호 안은 정의가 아니라 그림이 그려지는 말로, 15자 안팎.** 정확하지만 안 읽히는
  설명보다 조금 거칠어도 그림이 그려지는 쪽이 낫다. 정밀한 정의가 꼭 필요하면
  본문에서 따로 한 줄 더 쓴다.
- **약어는 풀어쓰기와 뜻을 둘 다** 넣는다: `PBC(Periodic Boundary Condition, 반복되는
  무늬의 한 칸만 계산하려고 양쪽 면을 묶는 조건)`.
- **한자어 전문용어도 같은 규칙**이다 (위 §7 표는 그대로 유효하다).
- **`## 쉬운 설명` 절은 예외** — 거기서는 전문용어 자체를 안 쓰므로 붙일 것도 없다.
- **표 안·코드블록 안에서는 생략해도 된다.** 칸이 좁으면 본문에서 한 번 풀고 표에서는
  원문만 쓴다.
- **뜻이 흔들리면 `docs/GLOSSARY.md`가 정본이다.** 새 용어를 처음 풀었으면 그 파일에
  추가한다. 같은 용어를 답변마다 다르게 설명하는 것이 가장 나쁘다.
