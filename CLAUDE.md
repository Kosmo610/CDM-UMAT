...
**해석 파일은 사용자 워크스테이션의 `E:\LTH\` 에 평평하게 둔다.**
경로는 언제나 `E:\LTH\파일명` 이며, 하위 폴더를 만들지 않는다.
...
- **명령을 적을 때 이 경로를 전제로 적는다.** 명령창 첫 줄은 `E:` 다음
  `cd \LTH` 이고, 그 뒤 명령은 전부 상대 경로로 쓴다 (그래야 복사가 안전하다).
- **zip 안의 최상위 폴더 하나**라는 §1 규칙은 그대로다. 사용자는 그 내용물이
  `E:\LTH\` 에 오도록 푼다.
