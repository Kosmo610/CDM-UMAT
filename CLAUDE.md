...
#### 7-1) ★ 전문용어는 **매번 괄호 풀이를 붙인다** (사용자 지정, 2026-08-23 — 세 브랜치 공통)

**논문·진행상황을 설명하는 모든 채팅 응답과 사용자 전달 문서(PDF 포함)에서,
전문용어가 나오면 `용어(쉬운 설명)` 형식으로 괄호 풀이를 단다.** 응답마다
그 용어가 처음 나오는 자리에는 반드시 단다 — "지난번에 설명했다"는 면제
사유가 아니다. 사용자는 여러 세션을 오가며 읽는다.

```
✅  균질화(여러 재료가 섞인 것을 평균 성질 하나로 뭉뚱그리는 계산)
✅  잔여강도(열충격을 겪은 뒤에도 남아 있는 버티는 힘)
❌  균질화 결과에 따라 …   (풀이 없음)
```

**영어 직역 번역투는 금지한다 — 뜻이 안 통한다.** 실제로 사용자가 지적한 말들:

| 쓰지 말 것 | 대신 |
|---|---|
| 고아 (orphan 직역) | 어디에서도 인용되지 않는 항목 / 본문에서 한 번도 안 불리는 표 |
| 처녀 상태 (virgin 직역) | 아직 아무 하중·손상을 겪지 않은 초기 상태 |

- 같은 계열(사람에 빗댄 직역: orphan·virgin 류)은 **표에 없어도 쓰지 않는다.**
  뜻을 풀어 쓴 서술로 대체한다.
- 저장소 안 코드·검사기 이름(예: `check_manuscript_citations` 의 orphan 검사)은
  내부 식별자이므로 유지하되, **사용자에게 설명할 때는 풀어 쓴다.**
- 제출본(`docs/CH1~7`)에 이런 번역투가 있으면 a1 이 순화한다.
