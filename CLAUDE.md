...
| **에이전트 3** | **총괄 · 감사** (코드 안 짬) | `claude/llm-task-decomposition-w41jpc` | 두 브랜치를 fetch 해서 읽음 | `paper/review/TO_A1.md` |
...
### ★ 세 번째 브랜치 — 총괄·감사 a3 (사용자 지정, 2026-08-12 역할 확정)

`claude/llm-task-decomposition-w41jpc` 가 **오케스트레이터**다. 2026-08-12 에
사용자가 역할을 못박았다:

> **a3는 총괄과 감사만 한다. 해석·코드 작업은 a2에게 시킨다.**

| a3가 하는 것 | a3가 하지 않는 것 |
|---|---|
| 진행 상황 파악과 보고 | UMAT·덱·후처리 코드 작성 |
| 전 게이트 실행과 장부 대조 | 해석 묶음(zip) 제작·전달 |
| 장 간 모순·인용·귀속 감사 | 카드값 결정 |
| **작업 지시서 발행** (`paper/review/TO_A1.md`) | 그 지시의 실행 |

- **a3가 코드를 고쳐야 할 것 같으면, 고치지 말고 지시서에 적는다.** 실행은 a2다.
- 예외는 **감사 도구 자신**뿐이다 — a3가 돌리는 검사기는 a3가 고쳐도 된다.
- 발신은 `paper/review/TO_A1.md` 다. a2·a1 의 `verification/review_inbox.py` 가
  **a3 브랜치의 이 경로를 직접 읽으므로**, a3가 자기 브랜치에 push 하면 전달된다.
  (파일명이 A1 인 것은 역사적 이유다 — 수신자는 a2·a1 **둘 다**이며, 지시서 안에서
  머리말로 대상을 나눈다.)

**a3 브랜치는 낡은 스냅샷이면 안 된다 (2026-08-12 수정).** 예전에 a3에는 `refs/`
가 없고 초안도 `paper/ch1·ch2·ch4` 뿐이라, **없는 것을 못 찾은 것을 없다고 판정하는**
사고가 실제로 났다($G_{f,1c}$ 귀속 강등). 이제 a3 브랜치는 해석 브랜치를 **병합해서
따라간다** — 감사자가 제출본과 `refs/` 전량을 직접 보고 판정한다. 감사 전에
`git merge origin/claude/thesis-csic-thermal-shock-5m53iv` 를 먼저 한다.
...
python3 postprocess/reheat_frames.py --selftest        # 재가열 프레임별 지연/재분배 분리 (r_t 최고수위 기준)
...
### ★ 전부 통과했다 ≠ 맞다 (2026-08-16, 한 주에 두 번 났다)

**초록이 곧 정답이라고 읽지 않는다.** 이 검사망은 「내가 묻기로 한 것」만
묻는다. 묻지 않기로 한 것은 **틀려도 조용하다.** 실제 사고 두 건:

| 무엇이 틀렸나 | 왜 안 걸렸나 |
|---|---|
| 병합에서 **장부 행 두 개가 사라졌다**(3160→3095) | 「합계 = 행들의 합」 검사는 **둘 다 같이 줄면 만족한다.** 그 통과를 보고 장을 짧아진 합에 맞추자 **손실이 자기일관적**이 됐다 |
| 제출본 제1장에 **충돌표시가 커밋됐다**(같은 문장·두 숫자) | 검사가 「'3207개 항목' 이라는 문자열이 있는가」를 본다. **있다.** 옆에 3034 가 같이 있어도 볼 이유가 없다 |

지켜야 할 것:

- **양(量)이 줄어드는 방향의 변경은 초록이어도 의심한다.** 행 수·항목 수·
  파일 수가 줄면 **왜 줄었는지 말할 수 있어야** 커밋한다.
- **문자열 포함(`in`)으로 짠 검사는 「있다」만 증명한다.** 「그것뿐이다」를
  물으려면 개수를 세거나 반대쪽(있으면 안 되는 것)을 같이 걸어야 한다.
- **숫자를 묻기 전에 「이 파일이 온전한 문서인가」를 먼저 묻는다.**
- 병합 충돌은 **한쪽을 통째로 고르지 않는다.** 목록·장부 파일은 **행 단위
  합집합**을 만들고, **합계보다 행 수를 먼저** 본다.
