C=======================================================================
C  UMAT_CSIC_RVE_DAMAGE_V2_2.for
C
...
C
      IF (NTENS.NE.6 .OR. NDI.NE.3 .OR. NSHR.NE.3) THEN
         WRITE(7,*) 'UMAT_CSIC_RVE_DAMAGE_V2_1: 3-D solids only.'
...
C
     ...
     1        NPROPS,DTIME,TEMP,DTEMP,PNEWDT,KSTEP)
...
C     Approximate energetic bookkeeping for diagnostics.
      ...
      END
C=======================================================================
      SUBROUTINE KYARN_UPDATE(EPS,STRESS,CTAN,SV,P,NP,DTIME,TEMP,
     1 DTEMP,PNEWDT,KSTEP)
      IMPLICIT NONE
...
C
      IF (ENABLE.GT.0.5D0 .AND. DBLE(KSTEP).LE.FREEZESTEP) THEN
         ...
         ELSE
            GAM=1.0D0
         END IF
         CALL KDAMAGE_TARGET(R1T,A1T,DMAX1,TAR)
...
C=======================================================================
     ...
     1 TEMP,DTEMP,PNEWDT,KSTEP,CELENT,COORDS)
      ...
      DOUBLE PRECISION WTLAG,WTU,MCRIT
