...
| `finding` | **양방향** | 발견한 것이 지금 저장소의 서술과 어긋남 |
| `changed` | **양방향** | 카드값·코드·규약이 바뀜 |
| `question` | 양방향 | 답이 필요함 |

> **한쪽 전용은 네 개뿐이다** (`band`·`value`·`ref`·`verdict`는 a1, `need`는 a2).
> 이 다섯만 **판정 권한**이 걸려 있어서 방향이 고정된다. 나머지는 양쪽 다 쓴다.
>
> `finding`·`changed`는 2026-08-06까지 a2 전용이었는데 **그것이 틀렸다.**
> a1의 첫 실전 묶음 7통이 전부 "형식 오류"로 찍혔다 — 논문을 읽고 우리 서술이
> 틀렸음을 발견하는 것은 문헌 담당이 하는 일 그 자체인데, 규약이 그것을 못 하게
> 막고 있었다. 규약이 "누가 무엇을 발견하는가"를 미리 정하고 있었고, 그 가정은
> 양쪽 모두에 대해 사실이 아니었다.

> ⚠️ **`blocking: true`면 `action`을 반드시 채운다.** 본문 끝에
> "판단해 달라"라고 써도 `action`이 비어 있으면 `blocking with no action`으로
> 찍힌다. 이건 잔소리가 아니라 실용이다 — 받는 쪽은 미처리 목록을 `action`으로
> 훑는다.
