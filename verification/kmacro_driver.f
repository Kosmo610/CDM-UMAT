...
C    STATEV(1..NSTATV)
C    EPS(1..6)
C    TEMP DTEMP DTIME FLDV CELENT KSTEP STIME
C  Output on stdout:
C    line 1  : STRESS(1..6)
...
C
      ...
      READ(*,*) (EPS(I),I=1,6)
      READ(*,*) TEMP,DTEMP,DTIME,FLDV,CELENT,KSTEP,STIME
C
      DO I=1,6
         STRESS(I)=0.0D0
         DO J=1,6
...
C
      IF (KIND.EQ.1) THEN
         NT=NINT(P(47))
         CALL KMACRO31(EPS,STRESS,CTAN,SV,P,NPROPS,NT,DTIME,TEMP,
     ...
     1        TEMP,DTEMP,PNEWDT,KSTEP,CELENT,NSTATV)
      ELSE
         ...
         ELSE
            NT=NINT(P(23))
         END IF
         CALL KMTRX31(EPS,STRESS,CTAN,SV,P,NPROPS,NT,DTIME,TEMP,
