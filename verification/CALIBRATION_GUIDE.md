...
| 32–33 | G1t,G1c (N/mm) | **12.5** | G1t = Ge Table 3; G1c는 근거 없이 동일값으로 둔 **본 연구의 가정** | 미세조정 |
...
> **굵은 값 = 논문에 없어 보정 대상.** `Vf=0.79194`(반올림 0.792)는 검증된 얀 섬유체적비.
...
- **eta (점성 정규화, matrix slot 10 / yarn slot 24):** 수렴이 어려우면 0.02→0.05
  로 올리고, 속도의존이 과하면 낮추세요. 해의 속도의존성이 무시될 만큼 작게 유지하는 것이
  원칙입니다(원 논문의 절 단위 귀속은 원문 미확보로 보류).
...
- **Xt=Vf·X_T300,t:** 종방향은 섬유지배(rule of mixtures). Vf=0.79194, X_T300,t=3580
  (Ge Table 2 = Zhang Table 1) → 2835. Xc 동일 논리로 1956.
- **G1t=12.5 (G1c는 동일값 가정):** G1t=12.5는 Ge 2018 Table 3(탄소/페놀 편조재)에 근거하며,
  압축 **G1c=12.5는 원 논문 근거 없이 동일값으로 둔 본 연구의 가정**이다(Gf,2t=1.0도 Table 3 수록).
  C/SiC는 더 취성이라
