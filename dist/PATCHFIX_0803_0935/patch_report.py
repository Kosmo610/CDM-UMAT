...
def set_label(odb, setname):
    """First node label of an assembly node set, or None."""
    try:
        st = odb.rootAssembly.nodeSets[setname.upper()]
    ...
    if not len(nodes):
        return None
    first = nodes[0]
    if hasattr(first, "__len__"):        # assembly sets give one tuple per instance
        if not len(first):
            return None
        first = first[0]
    return first.label
...
def driver_series(step, odb, k):
    ...
    hr = region_for(step, odb, "ConstraintsDriver%d" % k)
...
def report_patch(path):
    ...
    verdict("every entry of the 6x6 was recovered from the odb",
            ...
            "%d of 36 missing -- driver history output not found in the odb"
            % len(missing))
