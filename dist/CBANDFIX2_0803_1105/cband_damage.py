...
# WHEN *Depvar CARRIES NAMES, ABAQUS USES THEM.  make_patch_tests.py writes
#     *Depvar
#     20,
#     1, DMT, Matrix tensile damage
# and Abaqus then names the field output SDV_DMT, not SDV1.  The 2026-08-03
# run listed exactly that -- SDV_DMT, SDV_DMC, SDV_DMACT and the rest -- which
# is why the index-based lookup found nothing.  Names first, index second.
SDV_DAMAGE = ("SDV_DMT", "SDV1", "SDV_1", "SDV01")     # matrix tensile damage
SDV_ACTIVE = ("SDV_DMACT", "SDV5", "SDV_5", "SDV05")   # active matrix damage
...
def resolve_sdv(fr, names):
    """Find the field output for a state variable, or (None, None).

    `names` is tried in order: the *Depvar NAME first (SDV_DMT), then the
    index spellings (SDV1, SDV_1, SDV01), then a single "SDV" with components.
    Abaqus picks among these depending on whether *Depvar supplied names, and
    guessing which cost two round trips.
    """
    ...
    if "SDV" in keys:
        ...
        for cand in names:
            ...
            if digits:
                idx = int(digits)
                break
        return fr.fieldOutputs["SDV"], idx - 1
    return None, None


def last_damage(odb, names):
    """element label -> max state variable over its integration points, at the
    last frame that actually carries field output."""
...
def report(path):
    ...
    if dmg is None:
        print("  %-14s  none of %s found.  This frame carries:"
              % (name, ", ".join(SDV_DAMAGE)))
        for k in (avail or []):
            print("      %s" % k)
