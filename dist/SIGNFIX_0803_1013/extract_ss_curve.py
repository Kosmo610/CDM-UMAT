...
Periodic-BC convention in the ZHANG2022 .inp files (TexGen / Xia unified PBC):
  ...
  The sign is not a guess: PATCH_PBC recovers the analytic isotropic 6x6
  exactly with this convention and sign-flipped with the other one.
...
def extract(odbpath, Vuser=None):
    ...
    # Guard, not a correction.  With sigma = +RF/V a tension test already
    # reads positive, so this must never fire; if it does, something is wrong
    # upstream and silently flipping the curve would hide it.
    peak = max(rows, key=lambda r: abs(r[1]))
    if peak[1] < 0:
        print("  ** WARNING: peak stress is NEGATIVE (%.4f MPa).  With the"
              % peak[1])
        print("  ** verified convention sigma=+RF/V a tension test reads")
        print("  ** positive, so check the driver and the load direction.")
        print("  ** The curve is written AS COMPUTED, not flipped.")
