...
| **(23)(24)** | **매트릭스 자유에너지 = 탄성분 + 소성분 `G_m^p(ε̄_m^p)`** | 소성분 미반영 | ⚠️ **누락.** Ge 식 (23): *"the plastic part G_m^p is the contribution due to plastic hardening"* — 즉 매트릭스 A_m 유도 시 소성 소산이 계산에 들어가야 한다. 우리 `KABAND(Xt²/(2E)·CELENT, GMT, ...)`는 **탄성분만** 넣는다 |
...
| **(31)(32)(33)** | **Consistent tangent** `C_f,t = S_f⁻¹(d^v):[I − M_f(d^v)]`, `M_f = Σ ∂S_f/∂d · σ · Δt/(η+Δt) · ∂d/∂ε`; 매트릭스는 (32) + 등가강성 (33) | `CTAN(I,J)=CD(I,J)` — **secant** (`KYARN30:275-279`, `KMTRX30:437-`) | ❌ **미구현.** 손상 미분항 `M`이 없다. Ge p.92: *"To ensure the **quadratic convergence rate** of the Newton-Raphson method..."* → 우리는 연화 구간에서 **1차 수렴만** 얻는다. 이것이 M1 3차에서 η를 0.02→0.05로 올려야 했던 수렴 문제의 **구조적 원인일 가능성이 높다** |
...
> 이 중 실질적 성능 영향이 있는 것은 **(31)–(33) 미구현(수렴률)**과
> **(23)(24) 소성 소산 누락(매트릭스 A_m 과대추정)** 두 건이다.
