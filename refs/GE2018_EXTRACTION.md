...
2. ~~**매트릭스 A_m 유도 시 소성 소산 누락 (Ge 식 23–24)**~~ — **철회 (2026-08-12).**
   위 §D의 철회문이 정본이다. `G_m^p` 는 인자에 `d_m` 을 갖지 않으므로 식 (21)의
   `∂G/∂d_M` 에 **정확히 0** 을 기여한다. `KABAND` 가 탄성분만 넘기는 것이 옳다.
   지적을 지우지 않고 남기는 이유는 §D 철회문과 같다 — 왜 그렇게 판정했는지가
   나중에 필요해진다.
   **그 자리에 들어갈 진짜 한계**: 균열대 정규화는 손상 소산만 붙들고 소성 소산은
   놓아주므로 **총** 소산 에너지가 메시 의존이다(RVE 평균요소 16.1 %, 최대요소
   42.3 %). 이는 **Ge 정식화의 한계이며 본 구현의 오류가 아니다.**
   제7장 §7.3 한계 8번 · 제3장 §3.8-7 에 번호 붙은 한계로 등재되어 있고,
   `verification/plastic_dissipation_audit.py` 가 검사로 고정한다.
